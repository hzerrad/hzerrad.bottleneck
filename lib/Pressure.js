...
// Upward crossings only: a resource pinned at 90% for a minute is one spike,
// not thirty. That edge is when the culprit is still running.
function spikeOnsets(prevByKey, resources, spikeThreshold) {
  var out = []
  for (var i = 0; i < resources.length; i++) {
    var r = resources[i]
    if (r.ranks === false) continue
    var now = r.pressure * 100
    if (now < spikeThreshold) continue
    var prev = prevByKey ? prevByKey[r.key] : undefined
    if (prev === undefined || prev * 100 < spikeThreshold) out.push(r)
  }
  return out
}

function pressureByKey(resources) {
  var m = {}
  for (var i = 0; i < resources.length; i++) m[resources[i].key] = resources[i].pressure
  return m
}
