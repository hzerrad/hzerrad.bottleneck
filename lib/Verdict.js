...
// The hero is one Text with ElideRight, so an overlong sentence loses its
// tail -- the resource, which is the half worth reading. ps reports some
// comms long enough to trigger that ("node-MainThread"), and the name is the
// only variable-length part of the sentence, so the name is what gives.
var NAME_MAX = 12
...
function headline(o) {
  ...
  if (a && a.confident && VERBS[c.key]) {
    return {
      verdict: capitalize(shortName(a.name)) + " is " + VERBS[c.key] + " your " + c.label,
