...
function parseDf(text) {
  ...
  for (var i = 1; i < lines.length; i++) {
    ...
    // 1024-blocks and Used are already parsed past to reach Capacity and
    // Mounted on; keep them too rather than discarding the numerator a
    // capacity decision actually needs. A df variant that reports these as
    // "-" degrades to 0 rather than poisoning the display with NaN.
    ...
    seen[dev] = out.length
    out.push({ mount: f[6], usedPct: pct, sizeKb: sizeKb, usedKb: usedKb })
  }
  return out
}
