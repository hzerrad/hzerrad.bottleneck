...
// "<logical cpu> <physical core id>" per line, from cpuN/topology/core_id.
function parseCoreTopology(text) {
  var out = []
  var lines = String(text).trim().split("\n")
  for (var i = 0; i < lines.length; i++) {
    var f = lines[i].trim().split(/\s+/)
    if (f.length !== 2) continue
    var cpu = parseInt(f[0], 10)
    var core = parseInt(f[1], 10)
    if (isNaN(cpu) || isNaN(core)) continue
    out.push({ cpu: cpu, coreId: core })
  }
  return out
}

// Folds hyperthread siblings into their physical core — a core has the sensor.
function groupPhysicalCores(topo, classes) {
  var isP = {}
  var i
  for (i = 0; i < classes.p.length; i++) isP[classes.p[i]] = true
  ...
  for (i = 0; i < topo.length; i++) {
    var t = topo[i]
    if (!byCore[t.coreId]) {
      byCore[t.coreId] = { coreId: t.coreId, threads: [], kind: isP[t.cpu] ? "P" : "E" }
      order.push(t.coreId)
    }
    byCore[t.coreId].threads.push(t.cpu)
  }
  order.sort(function (a, b) { return a - b })

  // Intel core_ids are sparse (0,4,8… P, 24-31 E), so each core carries its
  // index within its class.
  var out = []
  var seq = { P: 0, E: 0 }
  for (i = 0; i < order.length; i++) {
    var c = byCore[order[i]]
    c.index = seq[c.kind]++
    c.name = c.kind + c.index
    out.push(c)
  }
  return out
}

function parseCoretempMap(labels) {
  var m = {}
  for (var i = 0; i < labels.length; i++) {
    var mm = String(labels[i].label).match(/^Core\s+(\d+)$/)
    if (mm) m[parseInt(mm[1], 10)] = labels[i].input
  }
  return m
}
